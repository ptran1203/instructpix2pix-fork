...
def sanity_check_resource(config):
    # Check if GPU is available
    if not torch.cuda.is_available():
        raise RuntimeError("No GPU detected. Please ensure a GPU is available for training.")

    # Check the precision
    precision = config.get("precision", 32)
    if precision != 16:
        raise RuntimeError("Only 16-bit precision is supported.")
    
    # Check gpus configuration
    gpus_cfg = config.get("gpus", None)
    if gpus_cfg is None:
        raise RuntimeError("No GPU configuration specified. Please specify gpus in lightning.trainer or via command line arguments.")
...
if __name__ == "__main__":
    ...
    try:
        ...
        sanity_check_resource(trainer_config)
        # default to ddp; we will disable it for single-GPU below.
        ...
        if isinstance(gpuinfo, int):
            ngpu_requested = max(1, gpuinfo)
        elif isinstance(gpuinfo, str):
            ngpu_requested = len([g for g in gpuinfo.strip(",").split(',') if g != ""])
            ngpu_requested = max(1, ngpu_requested)
        else:
            ngpu_requested = len(gpuinfo) if gpuinfo is not None else 1

        if ngpu_requested <= 1 and "accelerator" in trainer_config:
            print("Single-GPU run detected: disabling DDP accelerator.")
            del trainer_config["accelerator"]

        # Check the precision
        precision = trainer_config.get("precision", 32)
        assert precision == 16, "Only 16 bit precision are supported."
        ...
        if not cpu:
            ...
            if isinstance(gpus_cfg, int):
                ...
                ngpu = len([g for g in gpus_cfg.strip(",").split(',') if g != ""])
                ngpu = max(1, ngpu)
            else:
                ngpu = len(gpus_cfg) if gpus_cfg is not None else 1
        else:
            ngpu = 1
