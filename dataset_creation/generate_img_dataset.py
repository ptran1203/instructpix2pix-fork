...
def load_model_from_config(config, ckpt, vae_ckpt=None, verbose=False):
    ...
    if vae_ckpt is not None:
        ...
        vae_sd = torch.load(vae_ckpt, map_location=device)["state_dict"]
        sd = {
...
def main():
    ...
    parser.add_argument(
        "--skip-vae-ckpt",
        action="store_true",
        help="Skip loading a separate VAE checkpoint to reduce memory usage during startup.",
    )
    parser.add_argument(
        "--steps",
        type=int,
    ...
    model = load_model_from_config(
        ...
        ckpt=opt.ckpt,
        vae_ckpt=None if opt.skip_vae_ckpt else opt.vae_ckpt,
    )
    model.cuda().eval()
    print("Model loaded")
    ...
    use_clip_filter = not (
        opt.clip_threshold <= 0 and opt.clip_dir_threshold <= 0 and opt.clip_img_threshold <= 0
    )
    if use_clip_filter:
        clip_similarity = ClipSimilarity().cuda()
    else:
        clip_similarity = None
        print("CLIP filtering disabled (all CLIP thresholds <= 0).")
    ...
    with torch.no_grad(), torch.autocast("cuda"), model.ema_scope():
        ...
        for i, prompt in tqdm(prompts, desc="Prompts"):
            ...
            # Resume: skip prompts that already have saved outputs.
            if prompt_dir.joinpath("metadata.jsonl").exists():
                continue

            with open(prompt_dir.joinpath("prompt.json"), "w") as fp:
                json.dump(prompt, fp)

            cond = model.get_learned_conditioning([caption, prompt["output"]])
            results = {}
                ...
                while len(results) < opt.n_samples:
                    ...
                    if use_clip_filter:
                        clip_sim_0, clip_sim_1, clip_sim_dir, clip_sim_image = clip_similarity(
                            x0[None], x1[None], [caption], [prompt["output"]]
                        )
            ...
            if use_clip_filter:
                ...
                # Keep first generated samples without CLIP scoring/filtering.
                metadata = [(0.0, seed) for seed in results.keys()]
